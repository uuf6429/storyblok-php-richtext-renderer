...
class DefaultSchema implements SchemaInterface
{
        ...
        return static function ($node) {
            return [
                'single_tag' => [
                    [
                        'tag' => 'img',
                        'attrs' => array_intersect_key($node['attrs'], array_flip(['src', 'alt', 'title'])),
                    ]
                ]
            ];
        };
    ...
    protected function getAttrs(?array $node)
    {
        return $node['attrs'] ?? [];
    }

    protected function getLevel(?array $node): int
    {
        if ($node && array_key_exists('attrs', $node)) {
            $attrs = $node['attrs'];
            return (int)($attrs['level'] ?? 1);
        }
